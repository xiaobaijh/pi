...
describe("package commands", () => {
	...
	it("disapproves remembered project trust for list with --no-approve", async () => {
		mkdirSync(join(projectDir, ".pi"), { recursive: true });
		writeFileSync(join(projectDir, ".pi", "settings.json"), JSON.stringify({ packages: ["npm:@project/pkg"] }));
		new ProjectTrustStore(agentDir).set(projectDir, true);
		const logSpy = vi.spyOn(console, "log").mockImplementation(() => {});
	...
	it("blocks local package changes when project .pi is untrusted", async () => {
		...
		try {
			...
			expect(stderr).toContain("Project .pi is not trusted. Use --approve to modify local package config.");
